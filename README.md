...
* 一方、単純なタスク（清掃など）は **replay ベースの動作**として実装し、ユーザーが環境に合わせて簡単に調整可能
...
プロジェクトで使用したワークや治具、ロボットアームのパーツなど、合計13種類のモデルをプリントしました。また、モデル認識の頑健性を高めるため、意図的に出力に失敗した**「スパゲッティ（失敗ワーク）」**も学習データに使用しました。

# AMD_Robotics_Hackathon_2025_Automated_3D_Printing
...
3Dプリンタはオンラインで造形指示が可能ですが、造形後の「取り出し」と「清掃」に手動作業を要するため、その利便性が損なわれている現状があります。本プロジェクトでは、安価なロボットアーム「SO-101」を用い、これらの工程を自動化するオープンソースシステムを開発しました。 制御にはテレオペレーションによる模倣学習とリプレイ制御を機能ごとに最適化して併用し、システムの複雑化を回避しつつ、学習済みおよび未学習の物体に対する柔軟な対応力を実現しました。低コストかつシンプルな構成で、誰でも連続造形環境を構築できるユーザビリティの高いソリューションです。
...
* 汎用性が求められる動作（取り出し・物体操作）は **模倣学習** により柔軟性と再現性を確保
* 一方、単純なタスク（清掃など）は **リプレイベースの動作**として実装し、ユーザーが環境に合わせて簡単に調整可能
...
* **プリントモデル数**：15種＋意図的に用意した失敗ワーク

このプロジェクトで使用した、3Dプリンター出力によるモデルの一覧です。

プロジェクトで使用したワークや治具、ロボットアームのパーツなど、合計15種類のモデルをプリントしました。また、モデル認識の頑健性を高めるため、意図的に出力に失敗した**「スパゲッティ（失敗ワーク）」**も学習データに使用しました。

<img src="resources/63861.jpg" alt="使用した3Dプリントモデルの一覧写真" width="600"/>
